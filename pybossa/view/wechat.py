...
def login():  # pragma: no cover
    ...
    if ((ldap_enabled and no_login) or (not ldap_enabled)):
        callback = url_for('.oauth_authorized',
                           next=next_url,
                           no_login=no_login)
        return wechat.oauth.authorize(callback=callback)
...
def get_wechat_token():  # pragma: no cover
    ...
    return((current_user.info['wechat_token']['oauth_token'],
            current_user.info['wechat_token']['oauth_token_secret']))
